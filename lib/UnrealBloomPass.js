 ...
 */
	...
	class UnrealBloomPass extends THREE.Pass {
		...
		constructor( resolution, strength, radius, threshold ) {
			...
			// 原为 5 级：最粗一级只有 50x29，放大回屏幕后一个 mip 像素等于 32x31 像素，
			// 于是天体周围浮现出肉眼可见的方块。减到 3 级后最粗为 200x113（约 8x8 屏幕像素），
			// 块状伪影小到看不出来。改这个值必须同步改下方 kernelSizeArray、bloomFactors、
			// bloomTintColors，以及 composite shader 里引用的 blurTexture 数量。
			this.nMips = 3;
			...
			// 核在高斯权重尚未衰减到 0 的地方被截断，模糊结果的支撑域就成了一道硬边方框。
			// 支撑域 = kernelSize × 2 个 mip 像素，在 200x113 那级上 7 号核相当于 144 屏幕像素，
			// 于是天体周围出现可见的方形光晕。压到 3 后最外层支撑域约 48 像素且边缘权重极低。
			const kernelSizeArray = [ 3, 3, 3 ];
			...
			const bloomFactors = [ 1.0, 0.7, 0.4 ];
